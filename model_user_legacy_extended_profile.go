/*
Twitter OpenAPI

Twitter OpenAPI(Swagger) specification

API version: 0.0.1
Contact: yuki@yuki0311.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the UserLegacyExtendedProfile type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UserLegacyExtendedProfile{}

// UserLegacyExtendedProfile struct for UserLegacyExtendedProfile
type UserLegacyExtendedProfile struct {
	Birthdate *UserLegacyExtendedProfileBirthdate `json:"birthdate,omitempty"`
}

// NewUserLegacyExtendedProfile instantiates a new UserLegacyExtendedProfile object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUserLegacyExtendedProfile() *UserLegacyExtendedProfile {
	this := UserLegacyExtendedProfile{}
	return &this
}

// NewUserLegacyExtendedProfileWithDefaults instantiates a new UserLegacyExtendedProfile object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUserLegacyExtendedProfileWithDefaults() *UserLegacyExtendedProfile {
	this := UserLegacyExtendedProfile{}
	return &this
}

// GetBirthdate returns the Birthdate field value if set, zero value otherwise.
func (o *UserLegacyExtendedProfile) GetBirthdate() UserLegacyExtendedProfileBirthdate {
	if o == nil || IsNil(o.Birthdate) {
		var ret UserLegacyExtendedProfileBirthdate
		return ret
	}
	return *o.Birthdate
}

// GetBirthdateOk returns a tuple with the Birthdate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UserLegacyExtendedProfile) GetBirthdateOk() (*UserLegacyExtendedProfileBirthdate, bool) {
	if o == nil || IsNil(o.Birthdate) {
		return nil, false
	}
	return o.Birthdate, true
}

// HasBirthdate returns a boolean if a field has been set.
func (o *UserLegacyExtendedProfile) HasBirthdate() bool {
	if o != nil && !IsNil(o.Birthdate) {
		return true
	}

	return false
}

// SetBirthdate gets a reference to the given UserLegacyExtendedProfileBirthdate and assigns it to the Birthdate field.
func (o *UserLegacyExtendedProfile) SetBirthdate(v UserLegacyExtendedProfileBirthdate) {
	o.Birthdate = &v
}

func (o UserLegacyExtendedProfile) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UserLegacyExtendedProfile) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Birthdate) {
		toSerialize["birthdate"] = o.Birthdate
	}
	return toSerialize, nil
}

type NullableUserLegacyExtendedProfile struct {
	value *UserLegacyExtendedProfile
	isSet bool
}

func (v NullableUserLegacyExtendedProfile) Get() *UserLegacyExtendedProfile {
	return v.value
}

func (v *NullableUserLegacyExtendedProfile) Set(val *UserLegacyExtendedProfile) {
	v.value = val
	v.isSet = true
}

func (v NullableUserLegacyExtendedProfile) IsSet() bool {
	return v.isSet
}

func (v *NullableUserLegacyExtendedProfile) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUserLegacyExtendedProfile(val *UserLegacyExtendedProfile) *NullableUserLegacyExtendedProfile {
	return &NullableUserLegacyExtendedProfile{value: val, isSet: true}
}

func (v NullableUserLegacyExtendedProfile) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUserLegacyExtendedProfile) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


