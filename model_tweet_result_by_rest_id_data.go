/*
Twitter OpenAPI

Twitter OpenAPI(Swagger) specification

API version: 0.0.1
Contact: yuki@yuki0311.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the TweetResultByRestIdData type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &TweetResultByRestIdData{}

// TweetResultByRestIdData struct for TweetResultByRestIdData
type TweetResultByRestIdData struct {
	TweetResult ItemResult `json:"tweetResult"`
}

type _TweetResultByRestIdData TweetResultByRestIdData

// NewTweetResultByRestIdData instantiates a new TweetResultByRestIdData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTweetResultByRestIdData(tweetResult ItemResult) *TweetResultByRestIdData {
	this := TweetResultByRestIdData{}
	this.TweetResult = tweetResult
	return &this
}

// NewTweetResultByRestIdDataWithDefaults instantiates a new TweetResultByRestIdData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTweetResultByRestIdDataWithDefaults() *TweetResultByRestIdData {
	this := TweetResultByRestIdData{}
	return &this
}

// GetTweetResult returns the TweetResult field value
func (o *TweetResultByRestIdData) GetTweetResult() ItemResult {
	if o == nil {
		var ret ItemResult
		return ret
	}

	return o.TweetResult
}

// GetTweetResultOk returns a tuple with the TweetResult field value
// and a boolean to check if the value has been set.
func (o *TweetResultByRestIdData) GetTweetResultOk() (*ItemResult, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TweetResult, true
}

// SetTweetResult sets field value
func (o *TweetResultByRestIdData) SetTweetResult(v ItemResult) {
	o.TweetResult = v
}

func (o TweetResultByRestIdData) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o TweetResultByRestIdData) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["tweetResult"] = o.TweetResult
	return toSerialize, nil
}

func (o *TweetResultByRestIdData) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"tweetResult",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varTweetResultByRestIdData := _TweetResultByRestIdData{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varTweetResultByRestIdData)

	if err != nil {
		return err
	}

	*o = TweetResultByRestIdData(varTweetResultByRestIdData)

	return err
}

type NullableTweetResultByRestIdData struct {
	value *TweetResultByRestIdData
	isSet bool
}

func (v NullableTweetResultByRestIdData) Get() *TweetResultByRestIdData {
	return v.value
}

func (v *NullableTweetResultByRestIdData) Set(val *TweetResultByRestIdData) {
	v.value = val
	v.isSet = true
}

func (v NullableTweetResultByRestIdData) IsSet() bool {
	return v.isSet
}

func (v *NullableTweetResultByRestIdData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTweetResultByRestIdData(val *TweetResultByRestIdData) *NullableTweetResultByRestIdData {
	return &NullableTweetResultByRestIdData{value: val, isSet: true}
}

func (v NullableTweetResultByRestIdData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTweetResultByRestIdData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
