/*
Twitter OpenAPI

Twitter OpenAPI(Swagger) specification

API version: 0.0.1
Contact: yuki@yuki0311.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the UserProfessionalCategory type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UserProfessionalCategory{}

// UserProfessionalCategory struct for UserProfessionalCategory
type UserProfessionalCategory struct {
	IconName string `json:"icon_name"`
	Id       int32  `json:"id"`
	Name     string `json:"name"`
}

type _UserProfessionalCategory UserProfessionalCategory

// NewUserProfessionalCategory instantiates a new UserProfessionalCategory object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUserProfessionalCategory(iconName string, id int32, name string) *UserProfessionalCategory {
	this := UserProfessionalCategory{}
	this.IconName = iconName
	this.Id = id
	this.Name = name
	return &this
}

// NewUserProfessionalCategoryWithDefaults instantiates a new UserProfessionalCategory object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUserProfessionalCategoryWithDefaults() *UserProfessionalCategory {
	this := UserProfessionalCategory{}
	return &this
}

// GetIconName returns the IconName field value
func (o *UserProfessionalCategory) GetIconName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.IconName
}

// GetIconNameOk returns a tuple with the IconName field value
// and a boolean to check if the value has been set.
func (o *UserProfessionalCategory) GetIconNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IconName, true
}

// SetIconName sets field value
func (o *UserProfessionalCategory) SetIconName(v string) {
	o.IconName = v
}

// GetId returns the Id field value
func (o *UserProfessionalCategory) GetId() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *UserProfessionalCategory) GetIdOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *UserProfessionalCategory) SetId(v int32) {
	o.Id = v
}

// GetName returns the Name field value
func (o *UserProfessionalCategory) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *UserProfessionalCategory) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *UserProfessionalCategory) SetName(v string) {
	o.Name = v
}

func (o UserProfessionalCategory) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UserProfessionalCategory) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["icon_name"] = o.IconName
	toSerialize["id"] = o.Id
	toSerialize["name"] = o.Name
	return toSerialize, nil
}

func (o *UserProfessionalCategory) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"icon_name",
		"id",
		"name",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varUserProfessionalCategory := _UserProfessionalCategory{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varUserProfessionalCategory)

	if err != nil {
		return err
	}

	*o = UserProfessionalCategory(varUserProfessionalCategory)

	return err
}

type NullableUserProfessionalCategory struct {
	value *UserProfessionalCategory
	isSet bool
}

func (v NullableUserProfessionalCategory) Get() *UserProfessionalCategory {
	return v.value
}

func (v *NullableUserProfessionalCategory) Set(val *UserProfessionalCategory) {
	v.value = val
	v.isSet = true
}

func (v NullableUserProfessionalCategory) IsSet() bool {
	return v.isSet
}

func (v *NullableUserProfessionalCategory) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUserProfessionalCategory(val *UserProfessionalCategory) *NullableUserProfessionalCategory {
	return &NullableUserProfessionalCategory{value: val, isSet: true}
}

func (v NullableUserProfessionalCategory) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUserProfessionalCategory) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
