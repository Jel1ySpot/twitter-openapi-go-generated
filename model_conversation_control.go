/*
Twitter OpenAPI

Twitter OpenAPI(Swagger) specification

API version: 0.0.1
Contact: yuki@yuki0311.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ConversationControl type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConversationControl{}

// ConversationControl struct for ConversationControl
type ConversationControl struct {
	Mode string `json:"mode"`
}

type _ConversationControl ConversationControl

// NewConversationControl instantiates a new ConversationControl object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConversationControl(mode string) *ConversationControl {
	this := ConversationControl{}
	this.Mode = mode
	return &this
}

// NewConversationControlWithDefaults instantiates a new ConversationControl object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConversationControlWithDefaults() *ConversationControl {
	this := ConversationControl{}
	return &this
}

// GetMode returns the Mode field value
func (o *ConversationControl) GetMode() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Mode
}

// GetModeOk returns a tuple with the Mode field value
// and a boolean to check if the value has been set.
func (o *ConversationControl) GetModeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Mode, true
}

// SetMode sets field value
func (o *ConversationControl) SetMode(v string) {
	o.Mode = v
}

func (o ConversationControl) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConversationControl) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["mode"] = o.Mode
	return toSerialize, nil
}

func (o *ConversationControl) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"mode",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varConversationControl := _ConversationControl{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varConversationControl)

	if err != nil {
		return err
	}

	*o = ConversationControl(varConversationControl)

	return err
}

type NullableConversationControl struct {
	value *ConversationControl
	isSet bool
}

func (v NullableConversationControl) Get() *ConversationControl {
	return v.value
}

func (v *NullableConversationControl) Set(val *ConversationControl) {
	v.value = val
	v.isSet = true
}

func (v NullableConversationControl) IsSet() bool {
	return v.isSet
}

func (v *NullableConversationControl) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConversationControl(val *ConversationControl) *NullableConversationControl {
	return &NullableConversationControl{value: val, isSet: true}
}

func (v NullableConversationControl) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConversationControl) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


